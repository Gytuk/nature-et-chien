---
title: "Chien en randonnée : où est-il autorisé ou interdit en France ?"
description: "Parcs nationaux, forêts, réserves naturelles, littoral… Un guide clair sur les zones accessibles ou interdites, avec tableau récapitulatif, conseils pratiques et FAQ."
publishDate: 2026-05-02
updatedDate: 2026-05-18
category: avant-de-partir
tags: [législation, zones autorisées, parc national, forêt, laisse]
isAffiliate: false
featured: true
image: /images/avant-de-partir/interdiction-chien.jpg
imageAlt: "Panneau d'interdiction pour les chiens — zones autorisées et interdites en randonnée"
metaTitle: "Chien en randonnée : zones interdites et amendes en France"
metaDescription: "Parcs nationaux, régionaux, réserves naturelles, forêts, littoral : où votre chien est interdit en randonnée et amendes entre 35 € et 750 €."
---

Randonner avec votre chien est un plaisir partagé… à condition de savoir **où il est autorisé**, quelles règles s'appliquent et comment s'y préparer. Ce guide vous donne une vision claire des espaces ouverts ou fermés, avec les **conditions d'accès et les bons réflexes** à adopter.

## Parcs nationaux : zone « cœur »

Dans la plupart des parcs nationaux français, les chiens sont **interdits dans la zone cœur**, même tenus en laisse. L'objectif est de protéger une faune très sensible : **risque de dérangement des espèces, transmission de germes, prédation sur les nichées**.

Des dérogations limitées existent : chiens d'assistance, chiens de protection de troupeaux. Certains parcs prévoient également des exceptions ponctuelles sur des routes ou sentiers précis.

> 📎 **Source officielle :** [Règles générales et renvois par parc — parcsnationaux.fr](https://www.parcsnationaux.fr/fr/des-decouvertes/visiter-et-semerveiller/la-reglementation-et-les-conseils-pour-les-visiteurs)

## Parcs naturels régionaux (PNR)

En PNR, les chiens sont **généralement autorisés en laisse**. Des secteurs peuvent toutefois être plus restrictifs : **réserves associées, espaces naturels sensibles (ENS), alpages avec troupeaux**.

> 📎 **Trouver votre PNR :** [Carte-annuaire des 59 parcs — parcs-naturels-regionaux.fr](https://www.parcs-naturels-regionaux.fr/les-parcs/decouvrir-les-59-parcs)

## Réserves naturelles

Chaque réserve fixe son règlement selon la sensibilité du site. Certaines interdisent totalement les chiens, d'autres les autorisent en laisse sur des itinéraires balisés.

> 📎 **Consulter la liste des réserves :** [reserves-naturelles.org](https://reserves-naturelles.org/)

## Forêts (publiques et privées ouvertes au public)

Deux règles s'appliquent :

- **Toute l'année** : votre chien doit rester sous surveillance effective (ne pas s'éloigner au-delà d'environ 100 m / portée de voix).
- **Du 15 avril au 30 juin** : la laisse est **obligatoire** hors allées, chemins et sentiers balisés pour protéger la période de reproduction et de nidification.

> 📎 **Source :** [service-public.fr — Animaux en forêt](https://www.service-public.gouv.fr/particuliers/actualites/A17343)

## Littoral et plages

Il n'existe pas de règle nationale unique. L'accès dépend des arrêtés municipaux : interdiction saisonnière l'été, horaires ou secteurs dédiés, ou tolérance hors saison. Consultez la page « plages » de la commune ou la signalétique sur place avant de partir.

---

## Tableau récapitulatif

| Type d'espace | Chien autorisé ? | Conditions habituelles |
|---|---|---|
| Parc national – zone cœur | ❌ En général non | Interdit même en laisse ; exceptions limitées (assistance, pastoralisme) |
| Parc naturel régional (PNR) | ✅ Oui (souvent) | En laisse ; exceptions locales (réserves, ENS, alpages) |
| Réserve naturelle | ⚠️ Variable | Interdite ou autorisée en laisse sur itinéraires balisés |
| Forêt (toute l'année) | ✅ Oui | Sous surveillance (éviter la divagation au-delà de 100 m) |
| Forêt (15/04 → 30/06) | ✅ Oui | Laisse obligatoire hors allées/chemins/sentiers |
| Plages / littoral | ⚠️ Variable | Dépend d'un arrêté municipal (saisons / horaires / secteurs) |

---

<div class="not-prose my-8">
  <p class="text-sm italic text-gray-600 font-serif mb-4">Vous souhaitez vous équiper avant de partir en randonnée avec votre chien ? Consultez nos deux guides :</p>
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <a href="/equipement/gourde-gamelle-poche-eau-chien-comment-choisir" class="flex items-start gap-3 rounded-lg border border-gray-200 p-3 hover:bg-gray-50 transition-colors no-underline text-inherit">
      <img src="/images/equipement/choix-gourde-gamelle-ou-poche.jpg" alt="Chien qui boit dans une gamelle pliable lors d'une pause en randonnée" class="w-20 h-20 object-cover rounded flex-shrink-0 m-0" />
      <div>
        <p class="text-sm font-semibold text-gray-800 m-0 mb-1">Gourde, gamelle pliable ou poche à eau : Quel choix pour randonnée avec son chien ?</p>
        <p class="text-xs text-gray-500 m-0">Critères concrets pour choisir entre gourde avec abreuvoir, gamelle pliable et poche à eau selon la durée, la chaleur et le gabarit du chien.</p>
      </div>
    </a>
    <a href="/equipement/harnais-randonnee-chien" class="flex items-start gap-3 rounded-lg border border-gray-200 p-3 hover:bg-gray-50 transition-colors no-underline text-inherit">
      <img src="/images/equipement/chien-harnais.jpg" alt="Chien en randonnée avec harnais — guide de choix" class="w-20 h-20 object-cover rounded flex-shrink-0 m-0" />
      <div>
        <p class="text-sm font-semibold text-gray-800 m-0 mb-1">Randonnée avec son chien : comment choisir le harnais idéal ?</p>
        <p class="text-xs text-gray-500 m-0">Harnais classique, de traction ou multifonction : nos critères de choix, comparatif des meilleurs modèles et conseils d'ajustement pour randonner confortablement.</p>
      </div>
    </a>
  </div>
</div>

---

## Amendes et sanctions : ce que dit la loi, ce qui se passe en pratique

Les infractions liées aux chiens en randonnée ne relèvent pas d'un seul texte. Le montant varie selon le lieu, la situation et ce que le garde ou l'agent décide de retenir.

| Infraction | Amende forfaitaire | Maximum légal |
|---|---|---|
| Chien en divagation (hors espace protégé) | 35 € | 150 € |
| Chien sans laisse en forêt (15/04 – 30/06) | 135 € | 750 € |
| Chien dans le cœur d'un parc national | 68 € | 450 € |
| Dommages constatés à la faune | Variable | + dommages et intérêts |

**La divagation.** Un chien est légalement en état de divagation **dès qu'il dépasse 100 mètres** de son propriétaire ou n'est plus sous surveillance effective. **Cette règle s'applique partout en France**, en dehors de toute zone protégée, que la laisse soit obligatoire ou non. C'est l'arrêté ministériel du 16 mars 1955 et l'article R428-6 du code de l'environnement qui le définissent.

**Le chien sans laisse en forêt au printemps.** Entre **le 15 avril et le 30 juin**, la laisse est obligatoire hors allées et sentiers balisés. L'infraction relève d'une contravention de 4e classe. Le montant forfaitaire appliqué sur le terrain est de **135 €**. Le chiffre de 750 € souvent relayé dans les médias correspond au plafond légal maximal devant un juge, pas à l'amende distribuée par un agent de l'ONF.

**Le cœur d'un parc national.** La présence d'un chien dans la zone cœur, même tenu en laisse, est une infraction. Elle relève de l'article R331-64 du code de l'environnement, soit une contravention de 3e classe : **68 €** en forfaitaire, 450 € au maximum devant un juge. Les gardes sont assermentés et **patrouillent régulièrement sur les sentiers fréquentés en été**. La difficulté pratique est que la limite entre zone cœur (interdite) et zone d'adhésion (tolérée en laisse) n'est pas toujours signalée clairement sur le terrain. Sur certaines applications de randonnée, ces limites n'apparaissent qu'en activant le fond de carte OpenStreetMap en vue zoomée. **Avant toute sortie en montagne**, il est utile de vérifier son itinéraire sur le site officiel du parc ou sur Géoportail.

**Les arrêtés locaux.** Un maire peut ajouter des restrictions sur sa commune : laisse obligatoire, zones interdites, périodes spécifiques. Ces arrêtés doivent être affichés pour être opposables. En leur absence, la réglementation nationale sur la divagation reste en vigueur et s'applique partout.

---

## Que vérifier avant de partir avec son chien ?

### 1. Vérifiez le statut du lieu avant de partir

Avant chaque sortie, identifiez le type d'espace où vous allez : parc national, PNR, réserve naturelle, forêt ou littoral. Les règles changent fortement selon les milieux.

### 2. Adaptez votre matériel aux zones réglementées

Dans les espaces où la laisse est obligatoire, **une longe de 5 à 10 mètres** reste la solution la plus confortable pour votre chien tout en respectant la loi. Pour éviter les tractions et offrir un bon maintien, **un harnais adapté est préférable au collier**. <div class="inline-cta-list">
  <a href="/equipement/harnais-randonnee-chien" class="inline-cta-card">
    <img src="/images/equipement/chien-harnais.jpg" alt="Guide harnais de randonnée pour chien" loading="lazy" width="80" height="80" />
    <div class="inline-cta-text">
      <span class="inline-cta-title">Quel harnais choisir pour randonner avec son chien ?</span>
      <span class="inline-cta-desc">Types de harnais, critères de choix et comparatifs par profil et gabarit.</span>
    </div>
    <span class="inline-cta-arrow">→</span>
  </a>
</div>

### 3. Anticipez les périodes sensibles

Au printemps, de nombreuses espèces élèvent leurs petits ou nichent au sol. Il est conseillé de garder votre chien près de vous, même là où la laisse n'est pas obligatoire. Cette précaution limite le dérangement et vous évite de possibles sanctions.

### 4. En montagne, soyez attentif aux troupeaux

Sur les alpages, vous pouvez rencontrer des troupeaux et des chiens de protection. **Gardez votre chien en laisse**, contournez largement le troupeau et avancez calmement. Ces réflexes **facilitent la cohabitation et garantissent votre sécurité**.

### 5. Sur le littoral, consultez les règles locales

Chaque commune fixe ses règles. Un rapide coup d'œil aux panneaux d'accès ou au site de la mairie suffit souvent.

## Préparer sa première randonnée avec son chien

Si vous partez pour la première fois en randonnée avec votre chien, la réglementation n'est qu'un des points à anticiper. Retrouvez l'ensemble des vérifications à faire avant le départ — condition physique, équipement, hydratation — dans notre guide : [Première randonnée avec son chien : comment le préparer correctement ?](/avant-de-partir/premiere-randonnee-avec-son-chien/)

---

## FAQ

<div class="nc-faq-accordion">

  <details open>
    <summary>Où a-t-on le droit d'emmener son chien en randonnée ?<span class="faq-chevron"><svg viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg></span></summary>
    <div class="faq-answer"><p>De manière générale : les PNR (en laisse) et les forêts (sous contrôle, avec des règles plus strictes au printemps). En zone cœur de parc national, les chiens sont en principe interdits. Sur le littoral, cela dépend des arrêtés municipaux.</p></div>
  </details>

  <details>
    <summary>Existe-t-il un site qui liste tous les sentiers dog-friendly ?<span class="faq-chevron"><svg viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg></span></summary>
    <div class="faq-answer"><p>Non, il n'existe pas de base nationale répertoriant tous les itinéraires autorisant les chiens. Pour les parcs nationaux, <a href="https://www.parcsnationaux.fr">parcsnationaux.fr</a> renvoie vers chaque parc pour connaître les secteurs accessibles. Pour les PNR, la <a href="https://www.parcs-naturels-regionaux.fr">fédération des parcs</a> met à disposition une carte-annuaire.</p></div>
  </details>

  <details>
    <summary>Quand la laisse est-elle obligatoire en forêt ?<span class="faq-chevron"><svg viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg></span></summary>
    <div class="faq-answer"><p>La surveillance rapprochée s'applique toute l'année. Entre mi-avril et fin juin, la laisse est imposée hors allées, chemins et sentiers balisés pour protéger la nidification. En dehors de cette période, une longe reste utile si le rappel n'est pas fiable.</p></div>
  </details>

  <details>
    <summary>Que risque-t-on si l'on promène son chien dans un espace où il est interdit ?<span class="faq-chevron"><svg viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg></span></summary>
    <div class="faq-answer"><p>Une amende pouvant aller jusqu'à <strong>750 €</strong> pour violation des règles d'un espace protégé. Si le chien est en plus en état de divagation, <strong>une amende supplémentaire de 2ᵉ classe peut s'ajouter (22 à 75 €)</strong>.</p></div>
  </details>

  <details>
    <summary>Les chiens sont-ils autorisés en forêt domaniale en France ?<span class="faq-chevron"><svg viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg></span></summary>
    <div class="faq-answer"><p>Oui, les forêts domaniales sont ouvertes au public avec votre chien, sous surveillance effective toute l'année. Entre le 15 avril et le 30 juin, la laisse devient obligatoire hors allées, chemins et sentiers balisés pour protéger la nidification. En dehors de cette période, votre chien peut évoluer librement à condition de rester dans un rayon de 100 mètres et de répondre au rappel. Des restrictions locales peuvent s'appliquer selon les massifs : vérifiez la signalétique sur place.</p></div>
  </details>

  <details>
    <summary>Peut-on emmener son chien dans un parc national en zone périphérique ?<span class="faq-chevron"><svg viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg></span></summary>
    <div class="faq-answer"><p>Oui. La réglementation stricte ne concerne que la zone cœur du parc national, où les chiens sont interdits même tenus en laisse. La zone périphérique, appelée zone d'adhésion, est ouverte aux chiens en laisse. Le problème pratique : la limite entre les deux zones n'est pas toujours signalée clairement sur le terrain. Avant de partir, vérifiez votre itinéraire sur le site officiel du parc ou sur Géoportail en activant les couches de zonage.</p></div>
  </details>

</div>
